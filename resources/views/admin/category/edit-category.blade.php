                ...
                <form id="add_form" class="form-new-product form-style-1" action="{{route('admin.category.store')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow" type="text" placeholder="Category name" name="name"
                            tabindex="0" aria-required="true" required="" value="{{ $catSingle->name }}">
                    </fieldset>
                    <fieldset class="name">
                        ...
                        <input class="flex-grow" type="text" placeholder="Category Slug" name="slug"
                            tabindex="0" aria-required="true" required=""  value="{{ $catSingle->slug }}">
                    </fieldset>
                        ...
                        <div class="category-box">
                            <select name="parent_id" id="parent_id">
                                ...
                                @foreach ($categories as $category)
                                    ...
                                    <option @selected($catSingle->parent_id === $category->id) value="{{$category->id}}">{{$category->name}}</option>
                                        @foreach ($category->childs as $childs)
                                            <option @selected($catSingle->parent_id === $category->id) value="{{$childs->id}}"> - {{$childs->name}}</option>
                                            @foreach ($childs->childs as $kids)
                                                <option @selected($catSingle->parent_id === $category->id) value="{{$kids->id}}"> - - {{$kids->name}}</option>
                                                @foreach ($kids->childs as $gkids)
                                                    <option @selected($catSingle->parent_id === $category->id) disabled="disabled" value="{{$gkids->id}}"> - - - {{$gkids->name}}</option>
                                                @endforeach
                                            @endforeach
                                        @endforeach
                                    @endif
                    ...
                    <fieldset>
                        ...
                        <div class="upload-image flex-grow">
                            <div class="item" id="imgpreview" style="display:none">
                                <img src="{{$catSingle->image}}" class="effect8" alt="">
                            </div>
